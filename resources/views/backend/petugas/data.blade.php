...
<div class="modal fade" id="addPetugas" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  ...
  <script type="text/javascript">
    ...
    $('body').on('click', '.tombol-edit', function(e) {
        var id = $(this).data('id');
        $.ajax({
            url: '/admin/petugasAjax/' + id + '/edit',
            type: 'GET',
            success: function(response) {
                $("#editForm").prop('disabled', false);
                $("#addForm").prop('disabled', true);
                $('#editPetugas').modal('show');
                $('#editPetugas #username').val(response.result.username);
                $('#editPetugas #email').val(response.result.email);

                var idNew = response.result.id;
                $('.tombol-simpan-edit').click(function() {
                    $.ajax({
                        url: 'petugasAjax/' + idNew,
                        type:'PUT',
                        data:{
                            username : $('#editPetugas #username').val(),
                            email : $('#editPetugas #email').val()
                        },
                        success:function(response){
                            if(response.errors){
                                $.each(response.errors,function(key,value){
    ...
    $('#addPetugas').on('hidden.bs.modal',function(){
        $('#username').val('');
        $('#email').val('');
        $('#password').val('');
    });
